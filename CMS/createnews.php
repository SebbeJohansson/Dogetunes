...
if (isset($_SESSION['LAST_ACTIVITY']) && (time() - $_SESSION['LAST_ACTIVITY'] > 3000)) {
    ...
    session_unset(); 
    session_destroy();
    header ("Location: /CMS/login.php?s=inactive");
}
...
<body>
	...
	<section id="content">
		...
		<section id="contentholder">
			...
			<section id='hwrapp'><h3>Create News</h3></section>
			<form method="post" action="includes/doCreateNews.php" enctype="multipart/form-data">
                <label>Titel</label>
				...
				<div>
					<div>
						<textarea id="elm1" name="content" rows="15" cols="80" style="width: 80%"></textarea>
					</div>
				</div>
				<input type="hidden" name="author" value="<?php echo $admin['full_name']?>">
  				<a href="javascript:submitForm();"><input type="submit" name="submit" value="Create the News" class="submit"></a>
			</form>
